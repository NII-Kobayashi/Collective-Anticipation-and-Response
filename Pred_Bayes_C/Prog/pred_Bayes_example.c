...
//    Version 201217:  Output predicted time series *f_out
#include <stdio.h>
#include <math.h>
#include <stdlib.h>
#include <string.h> 
...
{ 
  if( argc != 10 )
    {   printf("Usage:./a.out  *f_in  *f_out  $k_p  $win  $alpha_c  $t_c  $a_n  $b_n  $t_n\n");  exit(1);   }
  ...
  //  Fitting Parameters: Bayes Method
  err=  pred_CycDec( N, win, k_p, x_p, p_cyc, p_pri, p_res);   
  printf("###  Error   %f\n", err);
  
  double  cyc, dec, exp_x, ome= M_PI/12.0;
  ...
  // for (i= 0;  i< N;  i++)   // Original Data
  //   {  fprintf( f_out, "%d   %.1f\n", i+1, x_p[i]);   }
  
  fclose(f_in);     fclose(f_out);     fclose(f_par);
  return 0;        
}
